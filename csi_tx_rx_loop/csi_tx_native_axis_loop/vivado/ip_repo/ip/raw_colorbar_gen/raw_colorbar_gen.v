...
( 
    ...
    input   [1:0]                 bayer_pattern , //0=RGGB, 1=GRBG, 2=GBRG, 3=BGGR
    ...
    begin
		...
		begin
			...
			begin
				if(pixcnt[0]==1'b0)
				begin
					raw_data <= ((color_cntr%300)<100)? {(word_width) {1'b0}} : ((color_cntr%300)<200) ? {(word_width) {1'b0}} : {(word_width) {1'b1}};
				end
				else if(pixcnt[0]==1'b1)
				begin
					raw_data <= ((color_cntr%300)<100)? {(word_width) {1'b0}} : ((color_cntr%300)<200) ? {(word_width) {1'b1}} : {(word_width) {1'b0}};
				end
			end
			...
			begin
				...
				begin
					raw_data <= ((color_cntr%300)<100)? {(word_width) {1'b1}} : ((color_cntr%300)<200) ? {(word_width) {1'b0}} : {(word_width) {1'b0}};
				end
			end
		end
