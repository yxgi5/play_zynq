...
int i2c_reg16_write(XIicPs *IicInstance,u8 i2c_slave_addr,u16 reg_addr,u8 data)
{
	...
	buf[1] = reg_addr&0xff;
	buf[2] = data;
	if(i2c_wrtie_bytes(IicInstance,i2c_slave_addr,buf,3) != XST_SUCCESS)
		return XST_FAILURE;
	return XST_SUCCESS;
}
...
{
	i2c_reg16_write(&IicInstance1,0x78>>1,addr,data);
}
...
{
	u8 ret;
	ret = i2c_reg16_read(&IicInstance1, 0x78>>1, addr);
