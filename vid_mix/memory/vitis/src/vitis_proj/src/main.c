...
//  vtiming_gen_run(&vtc, VIDEO_RESOLUTION_1080P, 0);
...
//Configure the CLKOUT0 DIV
  if( VM == XVIDC_VM_SVGA_60_P){
		Xil_Out32(clk_wiz.Config.BaseAddr + 0x208, 0xFA10); //800x600
  } else {
		Xil_Out32(clk_wiz.Config.BaseAddr + 0x208, 0x000A); //720p
  }
  Xil_Out32(clk_wiz.Config.BaseAddr + 0x25C, 0x3);
...
//      return(XST_FAILURE);
    ...
    //Configure the TPG0
    tpg_cfg(&tpg_inst, 1080, 1920, XVIDC_CSF_RGB, XTPG_BKGND_COLOR_BARS);
...
int main(void)
{
  ...
  *gpio_hlsIpReset = 1;
  ...
  i2c_init(&IicPs_inst, XPAR_XIICPS_0_DEVICE_ID, IIC_SCLK_RATE);
  XGpio_Initialize(&hdmi_reset, XPAR_AXI_GPIO_0_DEVICE_ID);
  ...
  i2c_reg8_write(&IicPs_inst, 0x72>>1, 0x08, 0x35);
  i2c_reg8_write(&IicPs_inst, 0x7a>>1, 0x2f, 0x00);
...
//  XClk_Wiz_CfgInitialize(&clk_wiz, &clk_wiz.Config, XPAR_CLK_WIZ_0_BASEADDR);
  Status = XClk_Wiz_dynamic_reconfig(&clk_wiz, XPAR_CLK_WIZ_0_DEVICE_ID, XVIDC_VM_1920x1080_60_P);
  if (Status != XST_SUCCESS)
  {
...
//  xil_printf("XVMonitor_IsVideoLocked(&vmon) = 0x%08x\r\n", XVMonitor_IsVideoLocked(&vmon));
...
//    ConfigClock(VidStream.VmId);
    Status = XClk_Wiz_dynamic_reconfig(&clk_wiz, XPAR_CLK_WIZ_0_DEVICE_ID, VidStream.VmId);
	if (Status != XST_SUCCESS)
	{
		xil_printf("XClk_Wiz0 dynamic reconfig failed.\r\n");
	}
...
//    vdma_config();
    ...
    if(Lock)
    {
      ...
      if(Status != 0)
      { //problems encountered in feature test
        ++FailCount;
      }
    }
    else
